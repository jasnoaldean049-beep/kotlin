...
internal class KTypeSubstitutor(
    ...
    fun substitute(type: KType, variance: KVariance = KVariance.INVARIANT): KTypeProjection {
        val substituted = substituteWithoutErasure(type, variance)
    ...
    private fun substituteWithoutErasure(type: KType, variance: KVariance = KVariance.INVARIANT): KTypeProjection {
        // Small optimization
    ...
    private fun copy(eraseToUpperBoundsAfterSubstitution: Boolean): KTypeSubstitutor =
        when {
            ...
            substitution.isEmpty() && eraseToUpperBoundsAfterSubstitution -> ERASE_ALL_ARGS_TO_PARAMS_UPPER_BOUNDS
    ...
    companion object {
        ...
        val ERASE_ALL_ARGS_TO_PARAMS_UPPER_BOUNDS = KTypeSubstitutor(emptyMap(), eraseToUpperBoundsAfterSubstitution = true)
...
private fun KType.eraseToUpperBoundsAndMakeItRawRecursively(seedTypeOfTheRecursionForDebug: KType = this): KType {
    ...
    with(type) {
        ...
        val lower = classifier.createType(arguments = newLowerBoundArguments, nullable = isMarkedNullable) // todo forward annotations?
