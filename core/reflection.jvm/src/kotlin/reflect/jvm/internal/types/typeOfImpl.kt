...
internal fun createPlatformKType(
    lowerBound: KType,
    upperBound: KType,
    isRawType: Boolean = false,
): KType =
    if (useK1Implementation) {
        val l = (lowerBound as DescriptorKType).type as SimpleType
        val u = (upperBound as DescriptorKType).type as SimpleType
        DescriptorKType(if (isRawType) RawTypeImpl(l, u) else KotlinTypeFactory.flexibleType(l, u))
    } else {
