...
internal class JvmCompilerArgumentsImpl() : CommonCompilerArgumentsImpl(), JvmCompilerArguments,
  ...
  public fun toCompilerArguments(arguments: K2JVMCompilerArguments = K2JVMCompilerArguments()): K2JVMCompilerArguments {
    ...
    if (X_WHEN_EXPRESSIONS in this) { try { arguments.whenExpressionsGeneration = get(X_WHEN_EXPRESSIONS)?.stringValue } catch(e: ClassCastException) { arguments.applyWhenExpressionsGeneration(get(X_WHEN_EXPRESSIONS)) }}
    ...
    if (JVM_DEFAULT in this) { try { arguments.jvmDefaultStable = get(JVM_DEFAULT)?.stringValue } catch(e: ClassCastException) { arguments.applyJvmDefaultStable(get(JVM_DEFAULT)) }}
