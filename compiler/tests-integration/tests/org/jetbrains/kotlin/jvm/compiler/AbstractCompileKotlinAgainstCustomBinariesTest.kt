...
abstract class AbstractCompileKotlinAgainstCustomBinariesTest : AbstractKotlinCompilerIntegrationTest() {
    ...
    fun testReleaseCompilerAgainstPreReleaseLibraryJs() = muteForK1 {
    ...
    fun testReleaseCompilerAgainstPreReleaseLibraryJsSkipPrereleaseCheck() = muteForK1 {
    ...
    */
    ...
    /**
     * Attempt to reproduce https://youtrack.jetbrains.com/issue/KT-83191
     */
    fun testLargeClasspathsPerformance() {
        ...
        val generationTime = measureTime {
            for (rootIndex in 0..<rootsCount) {
                ...
                val rootDir = File("$tmpdir/$rootName")
                ...
                val libraryFile = File(tmpdir, libraryName)
                compileLibrary(rootName, destination = libraryFile, dataDirectory = tmpdir, cleanupAfterCompilation = true)
        ...
        fun generateAndCompileKotlinFile(empty: Boolean, warmup: Boolean = false): Duration {
            ...
            val kotlinMainFile = File(tmpdir, fileName).apply {
            ...
            val compilationTime = measureTime {
                val (output, exitCode) = compileKotlin(
                    kotlinMainFile.absolutePath,
                    tmpdir,
                    classPaths,
                    expectedFileName = null,
                    ignoreTestDataDirectory = true
                )
                assertEmpty(output)
        ...
        println("Normal/empty diff: ${(normalFileCompileTime - emptyFileCompileTime).inWholeMilliseconds} ms" +
