...
internal object EagerResolveOfCollectionLiteral : ResolutionStage() {
    context(sink: CheckerSink, context: ResolutionContext)
    override suspend fun check(candidate: Candidate): Unit = context(context.typeContext) {
        ...
        for (atom in candidate.postponedAtoms) {
            ...
            val nonTvExpectedType = atom.expectedType?.takeUnless { it.typeConstructor() is TypeVariableTypeConstructorMarker } ?: continue
            val clBounds =
                CollectionLiteralBounds.NonTvExpected(atom, nonTvExpectedType.getClassRepresentativeForCollectionLiteralResolution())

            atom.analyzed = true
            runCollectionLiteralResolution(atom, candidate, clBounds, sink)
        }
    }
}
